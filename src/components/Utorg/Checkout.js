...
const Checkout = (props) => {
  ...
  useEffect(() => {
    checkTransactionStatus(
      { start: Date.now(), network, tx: payment_id },
      function (err, res) {
        if (err) {
          Swal.fire({
            icon: "error",
  ...
  const goBack = (e) => {
    ...
    if (isAndroid) {
      const url =
        "intent:/#Intent;scheme=https;package=com.velas.mobile_wallet;end";
      ...
      window.location.replace(
        parsed.env === undefined
          ? REDIRECT_URIS.wallet_mainnet
          : REDIRECT_URIS[parsed.env]
      );
    }
  };
  ...
  return (
    <Form className="input-form" action={REDIRECT_URIS[network]}>
      <motion.div
        ...
        <Form.Group>
          ...
          <div className="block-txt">
            ...
            <p className="row-txt payment_id" onClick={copyPaymentId}>
              {payment_id}
              <img src={Copy} alt="Copy" id="copy" className="copyIcon" />
            </p>
          </div>
        </Form.Group>
